...
auth0 = oauth.register(
    ...
    access_token_url='https://dev-fa25pxj9.us.auth0.com' + '/oauth/token',
    authorize_url='https://dev-fa25pxj9.us.auth0.com' + '/authorize',
    client_kwargs={
...
def log_out():
	...
	return redirect('https://dev-fa25pxj9.us.auth0.com' + '/v2/logout?' + urlencode(params))
...
def search_movie():
  ...
  response = Movie.query.filter(Movie.name.like('%{}%'.format(search_term)))
...
def search_actor():
  ...
  # search for "Music" should return "The Musical Hop" and "Park Square Live Music & Coffee"
  search_term = request.form.get('search_term', '')
  response = Actor.query.filter(Actor.name.like('%{}%'.format(search_term)))

  return render_template('pages/search_actor.html', results=response, search_term=request.form.get('search_term', ''))
